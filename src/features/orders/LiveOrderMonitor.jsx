      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs">
        <div>
          <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <ClipboardList className="text-rose-500" size={24} /> Live Kitchen
            Monitor
          </h1>
          <p className="text-xs md:text-sm text-slate-500 font-medium mt-0.5">
            Incoming merchant platform ticket logs in real-time execution
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-16 text-center shadow-xs flex flex-col items-center justify-center max-w-xl mx-auto space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium mt-1">
              No pending ticket streams waiting for execution orders at this
              millisecond.
            </p>
          </div>
        </div>
            ...
            >
              ...
              <div className="bg-slate-900 px-5 py-4 flex justify-between items-center text-white">
                <div>
                  ...
                  <span className="font-mono text-base font-black tracking-wider text-rose-400">
                    {order.orderId}
                  </span>
                </div>
              ...
              {order.tableNumber && (
                <div className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-[10px] font-black uppercase">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span
                            ...
                            title="Veg Indicator Item"
                          />
                          <span className="font-semibold text-slate-800">
                            {item.name}
                            <span className="text-rose-600 font-black ml-1.5 bg-rose-50 px-1.5 py-0.5 rounded-md text-[11px]">
                              x{item.quantity}
                            </span>
                          </span>
                        </div>
              ...
              <div className="px-5 py-3 flex justify-between items-center text-xs bg-slate-50/60 border-t border-slate-100">
                ...
                <span className="font-black text-base text-slate-900 tracking-tight">
                  ₹
                  {(order.total || order.subtotal || 0).toLocaleString("en-IN")}
                </span>
              </div>
