...
import { Plus, UtensilsCrossed, Edit2, Trash2, Layers, IndianRupee, Image } from 'lucide-react';
...
export default function MenuCatalog() {
  ...
  const { data: menuItems = [], isLoading } = useQuery({
    queryKey: ['menu-items'],
    queryFn: async () => {
      const res = await axios.get('http://localhost:5000/api/v1/menu/admin/items'); 
      return res.data.data || [];
    }
  });

  // Extract unique categories dynamically for food aggregator layout tabs
  const categoriesList = ['ALL', ...new Set(menuItems.map(item => item.category?.toUpperCase()).filter(Boolean))];

  // Filter items based on active top navigation pill selector
  const filteredMenuItems = activeCategoryFilter === 'ALL' 
    ? menuItems 
    : menuItems.filter(item => item.category?.toUpperCase() === activeCategoryFilter);
  ...
  const upsertMutation = useMutation({
    mutationFn: async (payload) => {
      if (editingItem) {
        return await axios.patch(`http://localhost:5000/api/v1/menu/admin/items/${editingItem._id}`, payload);
      }
      return await axios.post('http://localhost:5000/api/v1/menu/admin/items', payload);
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (itemId) => {
      return await axios.delete(`http://localhost:5000/api/v1/menu/admin/items/${itemId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['menu-items']);
    }
  ...
  const handleDeleteClick = (id) => {
    if (window.confirm("Are you sure you want to discard this item from live listing?")) {
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 font-medium mt-0.5">Configure categories, single dishes, and active operational items pricing</p>
        </div>
        <button 
      ...
      <Modal isOpen={isModalOpen} onClose={closeAndResetModal} title={editingItem ? "Modify Catalog Entry" : "Create New Menu Asset"}>
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <Input label="Dish Item Title" required placeholder="e.g., Spicy Paneer Tikka" value={name} onChange={(e) => setName(e.target.value)} />
          <Input label="Category Classification" required placeholder="e.g., Starters, Main Course, Desserts" value={category} onChange={(e) => setCategory(e.target.value)} />
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Selling Price (INR)" type="number" required placeholder="250" value={price} onChange={(e) => setPrice(e.target.value)} />
            <Input label="Dish Image Web URL Link" type="url" placeholder="https://images.unsplash.com/..." value={image} onChange={(e) => setImage(e.target.value)} />
          </div>

          <div className="space-y-1">
            <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1">Dish Description / Recipe Ingredients</label>
            <textarea 
              rows="3" 
          ...
          <div className="flex justify-end gap-2 pt-4 border-t border-slate-100 mt-6">
            ...
            >
              {upsertMutation.isLoading ? "Saving Asset..." : editingItem ? "Apply Modifications" : "Publish Item Live"}
            </button>
          </div>
        </form>
      </Modal>
