...
export default function MenuCatalog() {
  ...
  // 🔑 combo price ab sirf ek jagah compute hota h — teen jagah duplicate calculation hata di
      ...
      {filteredMenuItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMenuItems.map((item) => (
            <div
              key={item._id}
              className="relative bg-white rounded-2xl border border-slate-200/60 shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden group"
            >
              {item.isCombo && (
