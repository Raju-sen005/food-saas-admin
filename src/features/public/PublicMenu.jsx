...
const QuantityController = memo(function QuantityController({
  ...
  if (quantity === 0) {
    return (
      <button
        type="button"
        onClick={() => onAdd(id, name, price, type)}
...
export default function PublicMenu() {
  ...
  const {
    ...
    staleTime: 5 * 60_000, // menu 5 min tak fresh maano — baar baar refetch nahi
  ...
  return (
    <div className="min-h-screen bg-slate-100 sm:bg-slate-200/50">
      <div className="min-h-screen bg-slate-50 text-slate-900 pb-28 antialiased font-sans max-w-md sm:max-w-2xl lg:max-w-5xl mx-auto shadow-sm relative sm:border-x border-slate-200/40">
          ...
          <div className="px-4 sm:px-6 pb-4 bg-white">
            <div className="relative sm:max-w-md">
              <input
                ...
                className="w-full bg-slate-100 border border-slate-200 text-xs sm:text-sm font-medium py-3 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500/20"
              />
              {searchQuery && (
                <button
        ...
        {catalog?.offers?.length > 0 && (
          <div className="px-4 sm:px-6 py-4">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 -mr-4 pr-4 sm:mr-0 sm:pr-0 scrollbar-hide">
              {catalog.offers.map((o) => (
                <div
                  key={o._id}
                  className="min-w-[200px] sm:min-w-[220px] bg-gradient-to-br from-slate-900 to-slate-800 p-4 rounded-2xl text-white shadow-md border border-slate-700"
                >
              ...
              .map((categoryName) => (
                <div key={categoryName} className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {filteredCategoryItems[categoryName]?.map((item) => (
                      <div
                        key={item._id}
